...
//|                                                                  |
//|  Három sáv, PER-GYERTYA színbufferrel (DRAW_COLOR_HISTOGRAM2):    |
//|    • FENT (kék)      — M15 jelzési ablak nyitva                   |
//|    • KÖZÉPEN (zöld/piros) — SMA-irány (BUY/SELL)                  |
//|    • LENT (szürke)   — no-trade óra                              |
//|                                                                  |
//|  Adat: a Python által írt Common\Files\TFV_<Symbol>.csv `STATE`   |
//|  sorai (STATE;<bar_epoch>;<notrade>;<dir>;<window>). Az indikátor |
...
input int    SubWinHeightPx = 90;   // az al-ablak magassága px (0 = ne állítsd, kézi húzás)
// Három sáv az al-ablakban (0..1): kék M15-ablak FENT, SMA-trend KÖZÉPEN,
// szürke no-trade LENT.
input double BoxTop       = 0.95;   // M15 ablak (kék)
input double BoxBot       = 0.68;
input double RibbonTop    = 0.62;   // SMA-trend (zöld/piros)
...
void RefreshFromFile()
{
   ...
   while(!FileIsEnding(h))
   {
      ...
      {
         cnt = 0;
         ArrayResize(tms, 0); ArrayResize(nt, 0); ArrayResize(dr, 0); ArrayResize(wn, 0);
         continue;
      }
      ...
      ArrayResize(dr,  cnt + 1); ArrayResize(wn, cnt + 1);
      tms[cnt] = (datetime)StringToInteger(f[1]);
      nt[cnt]  = (int)StringToInteger(f[2]);
      dr[cnt]  = (int)StringToInteger(f[3]);
      wn[cnt]  = (int)StringToInteger(f[4]);
      cnt++;
   }
   FileClose(h);
